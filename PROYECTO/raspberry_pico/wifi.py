...
class WiFiManager:
    ...
    def await_response(self, timeout=5):
        ...
        self.debug_print(f"[await_response] Iniciando espera por respuesta con timeout={timeout}s")
        start_time = time.time()
        while time.time() - start_time < timeout:
            response = self.uart.read()
            if response:
                self.debug_print(f"[await_response] Respuesta recibida: {response}")
                return response
    ...
    def send_command(self, cmd, timeout=5):
        ...
        self.debug_print(f"[send_command] Enviando comando: {cmd}, con timeout={timeout}s")
        self.uart.write(cmd + '\r\n')
        response = self.await_response(timeout)
        if response:
            try:
                decoded_response = response.decode()
                self.debug_print(f"[send_command] Respuesta decodificada: {decoded_response}")
            except UnicodeError:
                self.debug_print("[send_command] Error al decodificar la respuesta. Mostrando en crudo:")
                self.debug_print(' '.join(f'{byte:02X}' for byte in response))
        else:
            self.debug_print(f"[send_command] Sin respuesta al comando '{cmd}'")
        return response
    ...
    def connect(self, max_retries=3):
        ...
        time.sleep(3)  # Pausa para permitir reinicio
        ...
        for attempt in range(max_retries):
            self.debug_print(f"[WiFiManager] Intento de conexión al WiFi ({attempt + 1}/{max_retries})...")
            response = self.send_command(f'AT+CWJAP="{self.ssid}","{self.password}"', 15)
            time.sleep(5)  # Pausa tras el intento de conexión

            if response and (b"WIFI CONNECTED" in response or b"WIFI GOT IP" in response):
                self.debug_print("[WiFiManager] Conexión exitosa.")
                break
            else:
                self.debug_print("[WiFiManager] Error al conectar, reintentando...")
        else:
            self.debug_print("[WiFiManager] Error: No se pudo conectar al WiFi tras varios intentos.")

        self.debug_print("[WiFiManager] Obteniendo información de red...")
        self.send_command('AT+CIFSR', 5)
